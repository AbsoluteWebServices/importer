...
func updateBinary() {
	...
	fmt.Printf("%sFailed to download update: %v\nstderr: %s%s\n", RED, err, string(out), REGULAR)
	return
    }
...
func exportDatabase(client *ssh.Client, sftpClient *sftp.Client, config *Config, dbConfig map[string]string) {
    ...
    runErr := <-errChan
    if runErr != nil {
	fmt.Printf("%sDatabase export failed: %v\nstderr: %s%s\n", RED, runErr, stderr.String(), REGULAR)
	os.Exit(1)
    }
    ...
    if stderr.Len() > 0 {
	fmt.Printf("%sWarning: mysqldump reported errors:\n%s%s\n", RED, stderr.String(), REGULAR)
    }

    fmt.Printf("%sDatabase export completed%s\n", GREEN, REGULAR)

    if out, err := exec.Command("gzip", "-d", dumpFile).CombinedOutput(); err != nil {
	fmt.Printf("%sDecompression failed: %v\nstderr: %s%s\n", RED, err, string(out), REGULAR)
	os.Exit(1)
    }
...
func downloadMedia(client *ssh.Client, sftpClient *sftp.Client, config *Config, mediaPath string) {
    ...
    runErr := <-errChan
    if runErr != nil {
	fmt.Printf("%sMedia download failed: %v\nstderr: %s%s\n", RED, runErr, stderr.String(), REGULAR)
	os.Exit(1)
    }

    bar.Finish()
    if stderr.Len() > 0 {
	fmt.Printf("%sWarning: tar reported errors:\n%s%s\n", RED, stderr.String(), REGULAR)
    }
